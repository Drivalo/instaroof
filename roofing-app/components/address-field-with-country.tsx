...
export const ADDRESS_CONFIRMATION_ERROR = "Please confirm your address is correct";
...
const AddressFieldWithCountry = forwardRef<AddressFieldHandle, AddressFieldWithCountryProps>(
  function AddressFieldWithCountry(
    ...
    useImperativeHandle(ref, () => ({
      getPlaceDetails: () => (placeConfirmed ? addressDetails : null),
      hasValidAddress: () => placeConfirmed && isValidPlaceDetails(addressDetails),
      validateForSubmit: () => {
        if (awaitingConfirmation && isValidPlaceDetails(addressDetails)) {
          setInlineError(ADDRESS_CONFIRMATION_ERROR);
          return false;
        }
    ...
    function handleConfirmYes() {
      if (!isValidPlaceDetails(addressDetails)) return;
      setAwaitingConfirmation(false);
      setConfirmed(true);
      setInlineError(null);
      notifyAddress(addressDetails);
    }
    ...
    return (
      <div ref={rootRef} className={`flex min-w-0 flex-1 flex-col gap-3 overflow-visible ${className}`.trim()}>
        <div className="overflow-visible">
          ...
          <div className="flex min-w-0 flex-col overflow-visible rounded-lg border border-border-subtle bg-[#1C1C1C] transition-colors focus-within:border-[#F5A623] sm:flex-row">
            ...
            {ready ? (
              <PlacesAutocompleteInput
                id={addressInputId}
                key={`address-${countryCode}-${inputResetKey}`}
          ...
          {awaitingConfirmation && addressDetails ? (
            <div className="mt-3">
              <p className="text-sm text-[#A0A0A0] leading-relaxed">
                Is this your property?{" "}
                <span className="text-[#FFFFFF]">{addressDetails.address}</span> —{" "}
                <button
                  type="button"
                  onClick={handleConfirmYes}
                  className="inline rounded-lg bg-[#F5A623] px-3 py-1.5 text-sm font-medium text-[#1C1C1C] transition-opacity hover:opacity-90"
                >
                  Yes, that&apos;s correct
                </button>{" "}
                <button
                  ...
                  className="inline rounded-lg border border-border-subtle bg-[#2A2A2A] px-3 py-1.5 text-sm text-[#FFFFFF] transition-colors hover:border-[#F5A623]"
                >
                  No, change address
                </button>
              </p>
